...
func init() {
	m.Register(func(db dbx.Builder) error {
		...
		if _, err := db.AddColumn("users", "tg_id", `NUMBER`).Execute(); err != nil {
			return err
		}
		migrateQuery := db.NewQuery(
			`
				...
				CREATE INDEX '_feed_last_fetch_idx' ON 'feed' ('last_fetch');
				...
				CREATE VIRTUAL TABLE post_idx USING fts5(id, tokens, context);
				CREATE TRIGGER post_ai AFTER INSERT ON post BEGIN
					INSERT INTO post_idx(id, tokens, context) VALUES (new.id, new.tokens, new.context);
			  	END;
				CREATE TRIGGER post_bd BEFORE DELETE ON post BEGIN
				  	DELETE FROM post_idx WHERE post_idx=old.id;
			    END;
				CREATE TRIGGER post_bu BEFORE UPDATE ON post BEGIN
				  	DELETE FROM post_idx WHERE post_idx=old.id;
			    END;
				CREATE TRIGGER post_au AFTER UPDATE ON post BEGIN
					INSERT INTO post_idx(id, tokens, context) VALUES (new.id, new.tokens, new.context);
			    END;
				CREATE VIRTUAL TABLE post_idx_row USING fts5vocab(post_idx, row);

				CREATE VIRTUAL TABLE feed_idx USING fts5(id, tokens, context);
				CREATE TRIGGER feed_ai AFTER INSERT ON feed BEGIN
					INSERT INTO feed_idx(id, tokens, context) VALUES (new.id, new.tokens, new.context);
			  	END;
				CREATE TRIGGER feed_bd BEFORE DELETE ON feed BEGIN
				  	DELETE FROM feed_idx WHERE feed_idx=old.id;
			    END;
		...
		/*
				CREATE TRIGGER post_ai AFTER INSERT ON post BEGIN
					...
					CREATE TRIGGER post_ad AFTER DELETE ON post BEGIN
					   INSERT INTO post_idx(post_idx, id, tokens, context) VALUES('delete', old.id, old.tokens, old.context);
					END;
					CREATE TRIGGER post_au AFTER UPDATE ON post BEGIN
					   INSERT INTO post_idx(post_idx, id, tokens, context) VALUES('delete', old.id, old.tokens, old.context);
					   INSERT INTO post_idx(id, tokens, context) VALUES (new.id, new.tokens, new.context);
					END;
						CREATE VIRTUAL TABLE feed_idx USING fts5(id, tokens, context, content='feed', content_rowid='id');
						CREATE TRIGGER feed_ai AFTER INSERT ON feed BEGIN
				     		INSERT INTO feed_idx(id, tokens, context) VALUES (new.id, new.tokens, new.context);
				   		END;
						CREATE TRIGGER feed_ad AFTER DELETE ON feed BEGIN
						   INSERT INTO feed_idx(feed_idx, id, tokens, context) VALUES('delete', old.id, old.tokens, old.context);
						END;
						CREATE TRIGGER feed_au AFTER UPDATE ON feed BEGIN
						   INSERT INTO feed_idx(feed_idx, id, tokens, context) VALUES('delete', old.id, old.tokens, old.context);
						   INSERT INTO feed_idx(id, tokens, context) VALUES (new.id, new.tokens, new.context);
